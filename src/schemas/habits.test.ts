...
describe('HabitFrequencySchema', () => {
  ...
  it('rechaza intervalo sin interval_days', () => {
    expect(() => HabitFrequencySchema.parse({ type: 'interval', target_per_period: 1 })).toThrow()
  })
  ...
  it('rechaza type desconocido', () => {
    expect(() => HabitFrequencySchema.parse({ type: 'mensual', target_per_period: 1 })).toThrow()
  })
})
...
describe('HabitSchema', () => {
  ...
  it('rechaza timestamp sin offset', () => {
    expect(() => HabitSchema.parse({ ...validHabit, created_at: '2026-06-27T10:00:00' })).toThrow()
  })
})
...
describe('HabitLogSchema', () => {
  ...
  it('rechaza log_date con formato incorrecto', () => {
    expect(() => HabitLogSchema.parse({ ...validLog, log_date: '27-06-2026' })).toThrow()
  })

  it('rechaza log_date con fecha inválida', () => {
    expect(() => HabitLogSchema.parse({ ...validLog, log_date: '2026-13-40' })).toThrow()
  })
...
describe('CreateHabitDraftSchema', () => {
  ...
  it('rechaza un draft sin color', () => {
    expect(() => CreateHabitDraftSchema.parse({ name: 'X', frequency: baseDaily })).toThrow()
  })
})
...
describe('CreateHabitLogDraftSchema', () => {
  it('acepta un log draft sin log_date (lo calcula el store)', () => {
    expect(CreateHabitLogDraftSchema.parse({ habit_id: validUuid })).toMatchObject({
      habit_id: validUuid,
    })
  })
})
...
describe('rowToHabit', () => {
  ...
  it('rechaza una fila que no cumple HabitRowSchema', () => {
    expect(() => rowToHabit({ ...dailyRow, color: 'no-es-hex' } as never)).toThrow()
  })
})
