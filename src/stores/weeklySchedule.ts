...
export const useWeeklyScheduleStore = defineStore("weeklySchedule", () => {
  ...
  function validateSlot(draft: { day_of_week: number; start_minutes: number;
                                  end_minutes: number }, ignoreSlotId?: string): ValidationResult {
  ...
  async function createBlock(
    ...
    const blockId = uuidv4(); const ts = nowIsoUtc();
  ...
  async function addSlot(blockId: string, draft: CreateScheduleSlotDraft): Promise<ScheduleSlot> {
    ...
    if (!existing) throw new Error("Bloque no encontrado");
    const slotId = uuidv4(); const ts = nowIsoUtc();
  ...
  async function updateSlot(slotId: string, patch: {
    ...
    blocksWithSlots.value = blocksWithSlots.value.map((b) =>
      b.id === bw.id
        ? { ...b, slots: b.slots.map((s) => (s.id === slotId ? updated : s)) }
        : b,
    );
  }
  ...
  return {
    blocksWithSlots, settings, loading, lastError,
    blocksByDay, wouldOverlapOnDay, validateSlot,
    loadAll, createBlock, updateBlock, deleteBlock,
