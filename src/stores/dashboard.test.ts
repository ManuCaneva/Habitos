...
describe('dashboard store', () => {
  ...
  it('resetea al layout por defecto', async () => {
    ...
    await flush()
    store.updateLayout([
      { i: 'habits', xPercent: 0.4, yPercent: 0.4, wPercent: 0.2, hPercent: 0.2 },
    ])
    store.resetLayout()
  ...
  it('moveTo clampa la posición para no salirse del contenedor', async () => {
    const store = useDashboardStore()
    await flush()
    store.updateLayout([{ i: 'a', xPercent: 0, yPercent: 0, wPercent: 0.5, hPercent: 0.4 }])
  ...
  it('resizeTo clampa tamaño mínimo y máximo', async () => {
    const store = useDashboardStore()
    await flush()
    store.updateLayout([{ i: 'a', xPercent: 0, yPercent: 0.5, wPercent: 0.4, hPercent: 0.4 }])
  ...
  it('addWidget coloca con tamaño mínimo cuando default size no cabe en ningún lado', async () => {
    const store = useDashboardStore()
    await flush()
    store.updateLayout([{ i: 'habits', xPercent: 0, yPercent: 0, wPercent: 1, hPercent: 0.8 }])
  ...
  it('addWidget loguea error si la grilla está completamente llena', async () => {
    const store = useDashboardStore()
    await flush()
    store.updateLayout([{ i: 'full', xPercent: 0, yPercent: 0, wPercent: 1, hPercent: 1 }])
