...
CREATE TABLE habits (
  ...
  -- Frecuencia polimórfica: exactamente una de las 3 "modas" activa
  ...
  -- Coherencia entre frequency_type y columnas asociadas
  CHECK (
    (frequency_type = 'daily'    AND interval_days IS NULL AND days_of_week IS NULL) OR
    (frequency_type = 'weekly'   AND interval_days IS NULL AND days_of_week IS NULL) OR
